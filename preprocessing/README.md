1.	read_cfo_nc.f90
	a.	This script reads L3 CFOSat netcdf files and converts wave number k to frequency vector and interpolates the spectra from cfosat spectral resolution to (frequency and  direction) to model’s spectral resolution. 
	b.	Make sure to compile it with your fortan compiler and create the read_cfo_nc file when you make changes.
	c.	Make sure to check beam angle if you are using other level of CFOSat data. (The variable names and sizes will also be different.)
	d.	Adjust the model specifications 
