...
USE WAM_SOURCE_OUTPUT_MODULE, ONLY: &
        SOURCE_OUTPUT
...
USE WAM_SOURCE_OUTPUT_MODULE, ONLY: CDT_SCR_OUT

USE WAM_GRID_MODULE,          ONLY: ONE_POINT, DEPTH_B,                        &
                                    AMOWEP, AMOSOP, AMOEAP, AMONOP,            &
				    XDELLO, XDELLA, NX, NY


USE WAM_OASIS_MODULE,         ONLY: USE_OASIS, USE_OASIS_ELEV_IN,              &
...
LOGICAL      :: NEW_DEPTH_OR_CURR, NEW_TOPO, NEW_CURR
LOGICAL, SAVE  :: FRST_T = .TRUE., FRST_C = .TRUE.
INTEGER :: CODE = 0
...
!         ------------------------------                                       !
   ...
   NEW_DEPTH_OR_CURR = NEW_TOPO .OR. NEW_CURR
...
!         -----------------------------                                        !
   ...
   CALL WAM_OASIS_CHECK_OUT

   PHYSICS: DO WHILE (CDTSOE.LE.CDTPRO)
     LCFLX = CDTINTT.EQ.CDTSOE .AND. ANY(CFLAG_P (59:62)).OR.use_oasis_force_source
...
&                      'CDTSOU = ',CDTSOU
      ...
      IF(use_oasis_wind_in)THEN
         CALL Wam_oasis_rec_atmo
      ELSE
...
!         -------------------------                                            !
   ...
   IF(use_oasis_bdy_in)THEN
     CALL Wam_oasis_rec_boundary
   ELSE IF (FINE) THEN
      CALL BOUNDARY_INPUT
      IF (ITEST.GE.2) THEN
...
!        --------------------------                                            !
   ...
   IF (COARSE) THEN
      ...
      END IF
   END IF
   if(use_oasis_nest_out)call wam_oasis_send_nest
...
!         ------------------------------------                                 !
   ...
   IF (CDTINTT.EQ.CDTPRO .OR. CDTSPT.EQ.CDTPRO) THEN
      ...
      CALL FLUSH1 (IU06)
   ELSE IF (USE_OASIS_FORCE_OUTPUT) THEN
      CALL MODEL_OUTPUT_CONTROL (fl3, 0, 0)
   END IF
