...
use wam_output_parameter_module,only: nf=>nout_p, titl_p
...
IF (ncid(0)<0) THEN
   ...
   vl(1,59) = 'phioc'
   vl(2,59) = 'energy flux to ocean'
   vl(3,59) = 'energy flux to ocean'
   vl(4,59) = 'kg/s/s'
   vmin(59) = -10.
   vmax(59) = 30.

   vl(1,60) = 'phiaw'
   vl(2,60) = 'energy flux from wind to waves'
   vl(3,60) = 'energy flux from wind to waves'
   vl(4,60) = 'kg/s/s'
   vmin(60) = 0.
   vmax(60) = 30.

   vl(1,61) = 'tauocx'
   vl(2,61) = 'x-comp. momentum flux into ocean'
   vl(3,61) = 'x-comp. momentum flux into ocean'
   vl(4,61) = 'kg/s/s'
   vmin(61) = -5.
   vmax(61) = 5.

   vl(1,62) = 'tauocy'
   vl(2,62) = 'y-comp. momentum flux into ocean'
   vl(3,62) = 'y-comp. momentum flux into ocean'
   vl(4,62) = 'kg/s/s'
   vmin(62) = -5.
   vmax(62) = 5.
   ...
   CALL Pf(NF90_CREATE(name,ior(ior(NF90_CLOBBER,NF90_SHARE),NF90_NETCDF4),ncid(0)))
   ...
   DO i=1,nf
      IF (flg(i)>0) THEN
         ...
         CALL Pf(NF90_DEF_VAR_CHUNKING(ncid(0),ncid(i),0,[nx,ny,1]))
         CALL Pf(NF90_DEF_VAR_DEFLATE(ncid(0),ncid(i),shuf,shuf,shuf))
         ...
         write (iu06,'(i3,1x,a25,a)') i,vl(2,i),trim(titl_p(i))
      ENDIF
   ENDDO
