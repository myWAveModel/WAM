...
INTERFACE mpi_scatter_block           !! scatter scalar block data field
   MODULE PROCEDURE mpi_scatter_block !! from a single process
END INTERFACE
PUBLIC mpi_scatter_block
...
if (irecv==0.or.petotal==1) then
  ...
  CALL MPI_Gatherv(field, nlen(irank), MPI_REAL, rfield, nlen, displ,          &
                   MPI_REAL, irecv-1, localcomm, ierr)
  IF (ierr/=0) THEN
    write (iu06,*) ' +++ error: SUB. mpi_gather_block'
...
subroutine mpi_scatter_block (isend, field, sfield)
...
!
!     gather scalar block data contained in the array field, distributed
!     across the different processes onto the single process isend
!
!     method :
!     --------
!
...
integer,                    intent(in)    :: isend !! process rank receiving the grid field
real, dimension(nijs:nijl), intent(inout) :: field !! containing the part be reveived
real, optional, dimension(1:nsea), intent(in) :: sfield !! the scattered field
...
if (isend==0.or.petotal==1) then
  comtime = MPI_WTIME()-comtime
  IF (.NOT. PRESENT(sfield)) THEN
    WRITE(iu06,*) ' +++ error: Sub. mpi_scatter_block'
    WRITE(iu06,*) ' +++ error: gathering process i = ', irank,' does not posess sfield'
    CALL abort1
  END IF
  field = sfield
  return
  ...
  CALL MPI_Scatterv(sfield, nlen, displ, MPI_REAL, field, nlen(irank),         &
                   MPI_REAL, isend-1, localcomm, ierr)
  IF (ierr/=0) THEN
    write (iu06,*) ' +++ error: SUB. mpi_scatter_block'
    write (iu06,*) ' +++ error: MPI-task ',irank,' -> MPI_Gatherv ierr = ',ierr
    call abort1
...
end subroutine mpi_scatter_block
...
!
   call MPI_send(zcombuf,len, MPI_REAL, irecv-1, itag, localcomm, ierr)
...
!
   ...
   do ip=1,petotal
      ...
      ELSE
         ...
         CALL MPI_recv(zcombuf, len, MPI_REAL, ip-1, itag, localcomm,  &
&                      istatus, ierr)
         IF (ierr/=0) then
            write (iu06,*) ' +++ error: Sub. mpi_gather_fl'
            write (iu06,*) ' +++ error: MPI_recv, ierr = ', ierr
...
subroutine mpi_scatter_fl (isend, itag, fl, sfl)
...
!         -------------------------------------------------
   ...
   call MPI_recv(zcombuf, len, MPI_REAL, isend-1, itag, localcomm, istatus, ierr)
...
!        load communication buffer
   ...
   call MPI_Bcast (zcombuf, len, MPI_real, isend-1, localcomm, ierr)
...
!        load communication buffer
   ...
   call MPI_Bcast (zcombuf, len, MPI_logical, isend-1, localcomm, ierr)
