...
INTERFACE PEAK_DIRECTION                   !! COMPUTATES PEAK DIRECTION.
...
CONST1 = 0.	! LOG(FC/FS)	Das ist Absicht !!
!  Moin Wolfgang, Moin Arno,
!
!  die Null schaltet tatsächlich die Tail-Korrektur aus. Bei der  ECMWF Version 36R4(habe ich 1012 geladen) ist sie noch nicht enthalten bei der Version 40R3  (bei mir geladen 2014) ist sie eingefügt. 
!
!  Das Problem beim mean-square-slope ist, dass das INT(K**2 F(f,th)) df dth von 0 bis oo nicht endlich ist, wenn mann f**-5 als tail ansetzt.
!
!  Nehmen wir Tiefwasser für die Tail an, dann is K**2 = (2PIf)**4/g**2 damit gilt für die "normale tail Korrektur ( bis auf Konstanten)
!  INT ((2PIf)**4/g**2*f**-5) df = (2PI)**4/g**2 INT (f**-1) df von fc bis oo = (2PI)**4/g**2 (Ln (oo)-ln(fc)) = oo
!
!  Die Tail Korrektur ist also nur beschränkt möglich und hat sich in der programmierten Form wohl nicht bewährt für eine ECMWF Anwendung auf Satelliten daten. 
!  Also schlechte Programmierung aber nicht falsch. Kommentar einfügen?
!
!  Bleibt gesund und weiter viel Spaß im home-office.
!  Heinz
...
SUBROUTINE PEAK_DIRECTION_B (F, PEAKD)
...
SUBROUTINE PEAK_DIRECTION_1 (F, PEAKD)
...
REAL    :: TEMP(SIZE(F,1),SIZE(F,3)), ACF(21,SIZE(F,1)), TLG(SIZE(F,1))                  !! WAM-MAX
...
! COMPUTE TEMPORAL AUTOCOVARIANCE FUNCTION                                               !! WAM-MAX

TLG = 0.3*T2                                                                             !! WAM-MAX
DO T = 1,21                                                                              !! WAM-MAX
   DO M = 1,ML                                                                           !! WAM-MAX
      DO IJ = 1,SIZE(TEMP,1)                                                             !! WAM-MAX
         ACF(T,IJ) = ACF(T,IJ) + COS(2.*PI*FR(M)*TLG(IJ))*TEMP(IJ,M)*DFIM(M)             !! WAM-MAX
      END DO                                                                             !! WAM-MAX
   END DO                                                                                !! WAM-MAX
   TLG = TLG + T2/20.                                                                    !! WAM-MAX
END DO
!
! ---------------------------------------------------------------------------- !         !! WAM-MAX
!                                                                              !         !! WAM-MAX
...
!        --------------------------------------------------------------------- !         !! WAM-MAX
      ...
      PHIST = ABS(MINVAL(ACF(:,IJ),DIM=1))/EMEAN(IJ)                                     !! WAM-MAX
