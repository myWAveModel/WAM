...
INTEGER :: IPHYS             !! PHYSICS PARAMETERISATION FOR INPUT AND OPEN OCEAN
                             ...
                             !!  2 : ST6, University of Melbourne source term package
...
ELSEIF (IPHYS.EQ.2) THEN
   WRITE (IU06,*) ' UNI. OF MELBOURNE SOURCE TERM PACKAGE ST6 FOR INPUT + OPEN OCEAN DISSIPATION'
ELSE
...
subroutine time_conversion(time_vector,time_string)
  ...
  REAL(kind=8), intent(out) :: time_vector !! time value seconds since 1950-01-01 
  CHARACTER(len=800) :: CDUMMY
  INTEGER :: MDAYS,yy,mo,RYY,RMO,RDY,EYY,EMO,EDY,RHH,RMM,RSS
  REAL(KIND=8)  :: RRC, REC, xhour, xmin
  xhour = 3600.
  xmin = 60
