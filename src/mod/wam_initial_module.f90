...
cdtpro=cdatea
IF(USE_OASIS_WIND_IN)CALL WAM_OASIS_REC_ATMO
IF(USE_OASIS_ELEV_IN)CALL WAM_OASIS_REC_TOPO(cdatea,gotfield)
IF(USE_OASIS_CURR_IN)CALL WAM_OASIS_REC_CURRENT(cdatea,gotfield)
...
IF (ICE_RUN.or.use_oasis_ice_in) THEN
   IF (ICE_RUN) THEN
      CALL GET_ICE
      IF (ITEST.GE.2) WRITE(IU06,*) '    SUB. PREPARE_START: GET_ICE DONE'
   END IF
   IF (use_oasis_ice_in) call Wam_oasis_rec_ice
...
NADV = MAX(IDELWI, IDELCI, IDELTI, IDELPRO, IDELT, 3600)
...
IF (IDELTI.LE.0 .OR. .NOT.TOPO_RUN) THEN
   ...
   IDELTO = 0
ELSE
   IF (.NOT.USE_OASIS_ELEV_IN)CD_TOPO_NEW = CDTPRO
   IF (IDELPRO.GT.IDELTO) THEN
      ...
      IDELTO = IDELPRO
   END IF
   IF (.NOT.USE_OASIS_ELEV_IN.AND.IDELPRO.LT.IDELTO) &
...
IF (IDELCI.LE.0 .OR. .NOT.CURRENT_RUN) THEN
   ...
   IDELCO = 0
ELSE
   IF (.NOT.USE_OASIS_CURR_IN)CD_CURR_NEW = CDTPRO
   IF (IDELPRO.GT.IDELCO) THEN
      ...
      IDELCO = IDELPRO
   END IF
   IF (.NOT.USE_OASIS_CURR_IN.AND.IDELPRO.LT.IDELCO) &
...
USE WAM_OASIS_MODULE,     ONLY: USE_OASIS,WAM_OASIS_WRITE_GRID
! ---------------------------------------------------------------------------- !
!                                                                              !
...
IF(USE_OASIS)CALL WAM_OASIS_WRITE_GRID
...
USE WAM_OASIS_MODULE,     ONLY: USE_OASIS_ELEV_IN
INTEGER  :: LEN
! ---------------------------------------------------------------------------- !
!                                                                              !
...
IF (LEN.GT.0 .AND. .NOT.TOPO_RUN .AND. .NOT.USE_OASIS_ELEV_IN) THEN
   WRITE (IU06,*) ' ****************************************************'
   WRITE (IU06,*) ' *						       *'
   WRITE (IU06,*) ' *	     FATAL ERROR SUB. PREPARE_FIRST_DEPTH.     *'
   WRITE (IU06,*) ' *	     =====================================     *'
...
!     3.1 DEPTH IS NOT DEFINED: USED BASIC DEPTH FROM PREPROC.                 !
   ...
   ELSE IF (USE_OASIS_ELEV_IN) THEN
...
USE WAM_OASIS_MODULE,     ONLY: USE_OASIS_CURR_IN
INTEGER  :: LEN
...
IF (COLDSTART) THEN
   ...
   IF (.NOT. CURRENT_RUN .AND. .NOT. USE_OASIS_CURR_IN) THEN
...
!     4.1 HOT START AND FILE DOES NOT EXIST.                                   !
      ...
      IF (CDCA.EQ.' ' .AND. USE_OASIS_CURR_IN) THEN
