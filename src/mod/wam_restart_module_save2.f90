...
real, allocatable, dimension (:,:,:) :: dbg	!wkdbg

!     local variables
!     ----------------                                                         !

integer            :: ifail, NSEA_R, ios, ierr
...
write (iu06,*)'CONNECT_RESTART_WK',L_DECOMP	!wkdbg
if (unformatted) then
...
allocate (dbg(ninf:nsup,1:kl,1:ml))	!wkdbg
if (irank==i_out_restart) then
   ...
   if (unformatted) then                       !! binary code
      IF (L_DECOMP) THEN
dbg(nijs:nijl,1,1)=u10;u10=-1	!wkdbg
	read (iu17) rfl(1:nsea,1,1)
	CALL mpi_scatter_block(i_out_restart,u10,rfl(1:nsea,1,1))
	CALL mpi_barrier(localcomm,ierr)
write(iu06,*)maxval(abs(dbg(nijs:nijl,1,1)-u10)),'u10 ufo',L_DECOMP	!wkdbg
	read (iu17) rfl(1:nsea,1,1)
	CALL mpi_scatter_block(i_out_restart,udir,rfl(1:nsea,1,1))
	CALL mpi_barrier(localcomm,ierr)
	read (iu17) rfl(1:nsea,1,1)
	CALL mpi_scatter_block(i_out_restart,tauw,rfl(1:nsea,1,1))
	CALL mpi_barrier(localcomm,ierr)
dbg(nijs:nijl,:,:)=fl3;fl3=-1	!wkdbg
write(iu06,*)'vor fl3',L_DECOMP	!wkdbg
	read (iu17) rfl(1:nsea,:,:)
	CALL mpi_scatter_fl(i_out_restart,131,fl3,rfl)
	CALL mpi_barrier(localcomm,ierr)
write(iu06,*)maxval(abs(dbg(nijs:nijl,:,:)-fl3)),'fl3 ufo',L_DECOMP	!wkdbg
      ELSE
dbg(nijs:nijl,1,1)=u10;u10=-1	!wkdbg
...
write(iu06,*)maxval(abs(dbg(nijs:nijl,:,:)-fl3)),'fl3 ufo',L_DECOMP	!wkdbg
      END IF
write(iu06,*) 'YUP YUP YUP 000', CDTA
      IF (CDTA.NE.' ') then
dbg(:,1,1)=depth;depth=-1	!wkdbg
	 IF (L_DECOMP) THEN
		write(iu06,*) 'YUP YUP YUP 01_1'
            READ (IU17) rfl(1:nsea,1,1)
	 ELSE
		write(iu06,*) 'YUP YUP YUP 01_2'
            READ (IU17) rfl(IJ2NEWIJ(1:nsea),1,1)
	 END IF
	 write(iu06,*) 'YUP YUP YUP 01_3'
	 CALL mpi_scatter_block(i_out_restart,depth(nijs:nijl),rfl(1:nsea,1,1))
	 CALL mpi_barrier(localcomm,ierr)
	 call mpi_exchng(depth)
      end if
write(iu06,*) 'YUP YUP YUP 02'
write(iu06,*)maxval(abs(dbg(:,1,1)-depth)),'depth ufo',L_DECOMP	!wkdbg
      IF (CDCA.NE.' ') then
	 IF (L_DECOMP) THEN
	 ...
	 END IF
	 CALL mpi_scatter_block(i_out_restart,u(nijs:nijl),rfl(1:nsea,1,1))
	 CALL mpi_barrier(localcomm,ierr)
	 call mpi_exchng(u)
	 CALL mpi_scatter_block(i_out_restart,v(nijs:nijl),rfl(1:nsea,2,1))
	 CALL mpi_barrier(localcomm,ierr)
	 call mpi_exchng(v)
      end if
...
write(iu06,*) 'YUP YUP YUP 03'
dbg(nijs:nijl,1,1)=u10;u10=-1	!wkdbg
   CALL mpi_scatter_block(i_out_restart, u10)
   CALL mpi_barrier(localcomm,ierr)
write(iu06,*) 'YUP YUP YUP 04'
write(iu06,*)maxval(abs(dbg(nijs:nijl,1,1)-u10)),'u10 ufo',L_DECOMP	!wkdbg
   CALL mpi_scatter_block(i_out_restart, udir)
   CALL mpi_barrier(localcomm,ierr)
   CALL mpi_scatter_block(i_out_restart, tauw)
   CALL mpi_barrier(localcomm,ierr)
dbg(nijs:nijl,:,:)=fl3;fl3=-1	!wkdbg
   call mpi_scatter_fl (i_out_restart,131,fl3)
   CALL mpi_barrier(localcomm,ierr)
write(iu06,*) 'YUP YUP YUP 050',cdta
write(iu06,*)maxval(abs(dbg(nijs:nijl,:,:)-fl3)),'fl3 ufo',L_DECOMP	!wkdbg
   if (cdta/='xxxxxxxxxxxxxx') then
dbg(:,1,1)=depth;depth=-1	!wkdbg
	write(iu06,*) 'YUP YUP YUP 06'
	 CALL mpi_scatter_block(i_out_restart, depth)
	 write(iu06,*) 'WAIT ALL START'
	 CALL mpi_barrier(localcomm,ierr)
	 write(iu06,*) 'WAIT ALL END'
write(iu06,*)maxval(abs(dbg(:,1,1)-depth)),'depth ufo',L_DECOMP	!wkdbg
   else
      cdta = ' '
   end if
write(iu06,*) 'YUP YUP YUP 07'
   if (cdca/='xxxxxxxxxxxxxx') then
	 CALL mpi_scatter_block(i_out_restart, u)
	 CALL mpi_barrier(localcomm,ierr)
	 CALL mpi_scatter_block(i_out_restart, v)
	 CALL mpi_barrier(localcomm,ierr)
         READ (IU17,*) rfl(1:nsea,1,1), rfl(1:nsea,2,1)
   else
     cdca = ' '
   end if
write(iu06,*) 'YUP YUP YUP 08'
endif
...
ELSE
   WRITE(IU06,*) ' YUP DEPTH FIELD WAS NOT STORED IN RESTART FILE ', CDTA
END IF
IF (CDCA.NE.' ') THEN
   WRITE(IU06,*) ' CURRENT FIELD DATE IS .............. CDCA  = ', CDCA
ELSE
   WRITE(IU06,*) ' CURRENT FIELD WAS NOT STORED IN RESTART FILE ', CDCA
END IF
...
END SUBROUTINE CONNECT_RESTART_WK
