...
read (iu17) rfl(1:nsea,:,:)
   ...
   IF (CDTA.NE.' ') then
      ...
      IF (L_DECOMP) THEN
         ...
         call HALO (rfl(1:nsea,1,1),depth)
      end if
   end if
   IF (CDCA.NE.' ') then
      ...
      IF (L_DECOMP) THEN
        ...
        call HALO (rfl(1:nsea,1,1),u)
        call HALO (rfl(1:nsea,2,1),v)
     end if
   end if
else                                        !! ascii code
   read (iu17,*) rfl(1:nsea,1,1)
...
SUBROUTINE HALO (FIELD_FULL, FIELD)

REAL, INTENT(IN)  :: FIELD_FULL(:)
REAL, INTENT(OUT) :: FIELD(:)
INTEGER ::ingb, iproc, kcount, ih, ij, nbufmax
...
FIELD(nijs:nijl) = FIELD_FULL(nijs:nijl)
...
do ingb = 1, ngbtope
   ...
   do ih = 1, ntope(iproc)
      ij = ijtope(ih,iproc)
      kcount =  kcount +1
      zcombufs(kcount,ingb) = FIELD_FULL(ij)
   enddo
enddo
do ingb = 1, ngbfrompe
