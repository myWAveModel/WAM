...
use wam_mpi_module,    only: irank, nijs, nijl, ipfgtbl, i_out_par, i_out_spec, localcomm

USE WAM_OASIS_MODULE,  ONLY: use_oasis_force_output,OASIS_OUTPUT_FLAGS
...
IF (USE_OASIS_FORCE_OUTPUT) THEN
   IF (iu20.eq.0)THEN
...
&                      indep(nijs:nijl))
   ...
   IF (IU20.GT.0) CALL WRITE_MODEL_OUTPUT (FL1, FL, IU20, IU25)
ELSE
   CALL COMPUTE_OUTPUT_PARAMETER (FL3, FL)
   IF (IU20.GT.0) CALL WRITE_MODEL_OUTPUT (FL3, FL, IU20, IU25)
END IF
