...
REAL    :: WAV(SIZE(F,1))
REAL    :: SBO(SIZE(F,1))
REAL    :: TEMP (SIZE(F,1),SIZE(F,2),SIZE(F,3))
...
REAL, DIMENSION(SIZE(F,1),SIZE(F,2),SIZE(F,3)) :: TEMP
...
IF (FIRST) CALL SAVE_SOURCE (SL,2)
FIRST = .FALSE.
...
SUBROUTINE SNONLIN (F, SH, FL, DEPTH, AKMEAN)
...
REAL, INTENT(INOUT) :: SH(:,:,:)    !! TOTAL SOURCE FUNCTION ARRAY.
...
REAL, DIMENSION(SIZE(F,1),SIZE(F,2),SIZE(F,3)) :: SL
...
SH = SL + SH
CALL SAVE_SOURCE(SL,3)

END SUBROUTINE SNONLIN

! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ !

SUBROUTINE SOURCE_PHILLIPS (SH, USTAR, UDIR, DEPTH, INDEP)
...
REAL,   INTENT(INOUT)        :: SH(:,:,:)  !! TOTAL SOURCE FUNCTION.
REAL,   INTENT(IN)           :: USTAR(:)   !! FRICTION VELOCITY
REAL,   INTENT(IN)           :: UDIR(:)    !! WIND DIRECTION
...
REAL    :: CONST1
REAL    :: TEMP(1:SIZE(SH,1)), FPM(1:SIZE(SH,1)), KD
REAL    :: TPHOLD(1:SIZE(SH,1),1:SIZE(SH,2))
REAL    :: SL(1:SIZE(SH,1),1:SIZE(SH,2),1:SIZE(SH,3))
