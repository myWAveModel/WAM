...
!					 MPI_UNDEFINED,MPI_COMM_NULL,MPI_INTEGER
  ...
  !USE WAM_OUTPUT_PARAMETER_MODULE,	ONLY:	TITL_P !! ,TITL_SCR !! RH: not used yet
  USE WAM_OUTPUT_PARAMETER_MODULE,	ONLY:	NOUT_P,TITL_P !! ModR07
...
!
    ELSE				! Decomposition is 2D (and petot > 1 !!!)
        ...
        CALL MPI_ALLGATHER(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,ofs,1,mpi_integer,localcomm,ierror)
...
! define variables for coupler
    ...
    DO k=1,nvars		! pass variable info to coupler
        IF(k<vbdyi.OR.vbdyi<k)THEN
            CALL oasis_def_var(var_id(k),vnamen(k),part_id,nd,iot(k),ns(1:2),OASIS_Real,ierror)
...
! WAM nest output partitioning
    ...
    IF(use_oasis_wind_in)THEN			! set WAM model variables
        ...
        cdatewo=cda
        CALL INCDATE(cdatewo,ioi(vatmi))        !! ModR07: Bugfix coupling intervals
        idelwi=ioi(vatmi)                       !! ModR07
        IF(idelwi>idelwo)idelto=3600
        ALLOCATE(bug(size(buf)))		! buffer for v
        write(iu06,*) 'oasis_wind_set: ',cda,'; ',cdatewo,'; ',ioi(vatmi),'; ',idelwi
    ENDIF
    IF(use_oasis_elev_in)THEN			! set WAM model variables
        cdta=cdatea				! for water level
        cd_topo_new=cdta
        CALL INCDATE(cd_topo_new,ioi(vgcmi))    !! ModR07: Bugfix coupling intervals
        idelti=ioi(vgcmi)                       !! ModR07
        IF(idelti>idelto)idelto=3600
        write(iu06,*) 'oasis_elev_set: ',cdta,'; ',cd_topo_new,'; ',ioi(vgcmi),'; ',idelti
    ENDIF
    IF(use_oasis_curr_in)THEN			! set WAM model variables
        cdca=cdatea				! for current
        cd_curr_new=cdca
        CALL INCDATE(cd_curr_new,ioi(vgcmi+2))  !! ModR07: Bugfix coupling intervals
        idelci=ioi(vgcmi+1)                     !! ModR07
        IF(idelci>idelco)idelco=3600
        write(iu06,*) 'oasis_curr_set: ',cdca,'; ',cd_curr_new,'; ',ioi(vgcmi+1),'; ',ioi(vgcmi+2),'; ',idelci
    ENDIF
    IF(use_oasis_force_output)THEN		! set flags for WAM outputs
        !ALLOCATE(oasis_output_flags(nout_p,3)) !! ModR07: Changed to static allocation
        !oasis_output_flags(:,1)=.false.        !! ModR07
...
CONTAINS
	...
	IF(a<b)THEN		! output for this nest is done in this process
	    ...
	    CALL oasis_def_var(var_id(i2),vnamen(i2),p,nd,iot(i2),ns,OASIS_Real,ierror)
	    IF (ierror/=0)call Oaa('oasis_def_var','oasis_def_var failed')
        ENDIF
	an(i1)=a
	bn(i1)=b
...
!
  SUBROUTINE Wam_oasis_rec_atmo(cdat)                   !! ModR07: Add time parameter cdat
   ...
   INTEGER		:: isec
    CALL DIFDATE(CDATEA,cdat,isec)		! seconds since model start !! ModR07: CDTpro -> cdat
    IF(ITEST.GE.7)WRITE(IU06,*)'Start Wam_oasis_rec_atmo',rank,cdat,isec    !! ModR07: CDTpro -> cdat
	...
	cdatewo=cda
	CALL INCDATE(cdatewo,ioi(vatmi))                !! ModR07: Bugfix ioi(var_id(*)) -> ioi(*)
	...
	ELSE						! at start time take restart if available
        ...
        ENDIF
      ENDIF
    IF(ITEST.GE.7)WRITE(IU06,*)'End   Wam_oasis_rec_atmo',rank,cdat,isec,ierror !! ModR07: CDTpro -> cdat
...
!
  SUBROUTINE Wam_oasis_rec_boundary(cdat)               !! ModR07: Add time parameter cdat
   ...
   LOGICAL		:: gotdata,fl3iavail=.false.
    km=kl*ml
    CALL DIFDATE(cdatea,cdat,isec)		! seconds since model start            !! ModR07: CDTpro -> cdat
    IF(ITEST.GE.7)WRITE(IU06,*)'Start Wam_oasis_rec_boundary ',cdat,isec,bdy_in,bdyuse !! ModR07: CDTpro -> cdat
    if(bdy_in)then
	...
	if(allocated(fl3i))then
      ...
      ENDIF
    IF(ITEST.GE.7)WRITE(IU06,*)'End   Wam_oasis_rec_boundary ',cdat,isec !! ModR07: CDTpro -> cdat
...
SUBROUTINE Wam_oasis_rec_topo(cdat,gotfield)
    ...
    IF(ierror/=OASIS_Ok)THEN	! if data obtained
        ...
        IF((isec>=0).OR.coldstart)THEN	! map oasis data to wam partition !! ModR07: '>' -> '>='
            IF(petot>1)THEN
        ...
        cd_topo_new=cdta
        CALL INCDATE(cd_topo_new,ioi(vgcmi))                              !! ModR07: Bugfix ioi(var_id(*)) -> ioi(*)
    ENDIF
    !!=== ModR07: Updated Error handling & unified structure ===
...
SUBROUTINE Wam_oasis_rec_current(cdat,gotfield)
    ...
    IF(ierror/=OASIS_Ok)THEN		! if data obtained
        IF(ITEST.GE.6)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_current U',cdat,isec,ierror
        IF((isec>=0).or.coldstart)THEN	! map oasis data to wam partition !! ModR07: '>' -> '>='
            ...
            gotfield=.true.
        ELSE
            IF(ITEST.GE.6)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_current U-values discarded'
        ENDIF
    ENDIF
    ...
    IF(ierror/=OASIS_Ok)THEN		! if data obtained
        IF(ITEST.GE.6)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_current V',cdat,isec,ierror
        IF((isec>=0).or.coldstart)THEN	! map oasis data to wam partition !! ModR07: '>' -> '>='
            ...
            gotfield=.true.
        ELSE
            IF(ITEST.GE.6)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_current V-values discarded'
            IF(gotfield)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_current only got u but not v !'
        ENDIF

        cdca=cdatea			! fill in WAM model variables
        CALL INCDATE(cdca,isec)
        cd_curr_new=cdca
        CALL INCDATE(cd_curr_new,ioi(vgcmi+2))                            !! ModR07: Bugfix ioi(var_id(*)) -> ioi(*)
    ENDIF
    !!=== End ModR07: re-structure ===
...
SUBROUTINE Wam_oasis_rec_ice(cdat)                      !! ModR07: Add time parameter cdat
    ...
    CALL DIFDATE(CDATEA,cdat,isec)			! seconds since model start !! ModR07: CDTpro -> cdat
    IF(ITEST.GE.7)WRITE(IU06,*)'Start Wam_oasis_rec_ice',rank,cdat,isec             !! ModR07: CDTpro -> cdat
    CALL oasis_get(var_id(vicei),isec,buf,ierror)	! only ice for this process
    IF(ierror.NE.OASIS_Ok.AND.ierror.LT.OASIS_Recvd) &
        call Oaa('oasis_get for ice information','oasis_get ice failed')
    IF(ierror/=OASIS_Ok)THEN				! if data obtained fill in WAM model variables
        IF(ITEST.GE.6)WRITE(IU06,*)'SUBROUTINE Wam_oasis_rec_ice ',cdat,isec,ierror,COUNT(buf(ix)>0) !! ModR07: CDTpro -> cdat
        ...
        IF(petot>1)THEN					! needs to collect ice from other processors
            ...
            CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,loi,rcc,ofs,mpi_integer,localcomm,ierror)	! gather ice data from all processors
            IF(ierror/=0) &
    ...
    ENDIF
    IF(ITEST.GE.7)WRITE(IU06,*)'End   Wam_oasis_rec_ice',rank,cdat,isec,ierror,N_ICE !! ModR07: CDTpro -> cdat
